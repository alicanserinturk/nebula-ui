<template>
  <app-module class="">
    <app-module-header>
      <div slot="action">
        <button v-if="$api.has('announcement_edit')" @click="$router.push($route.path + '/edit')" class="btn btn-sm btn-rounded btn-light i-con-h-a">
        ...
        <div class="app-module-main">
          <app-card>
            ...
            <app-label icon="el-icon-view" label="Görünürlük">
              <span v-if="form.is_global">Tüm müşterilerin kullanıcıları tarafından görülebilir.</span><span v-else>Yalnızca kendi kullanıcılarınız tarafından görülebilir.</span>
            </app-label>
            <app-label icon="el-icon-alarm-clock" label="En Üste Sabitle">
              <span v-if="form.is_pinned">Sabitlendi</span><span v-else>Sabitlenmedi</span></app-label>
