<template>
	<app-module class="">
		<app-module-header>
			<div slot="action">
				<button
					v-if="$api.has('result_code_edit') && !form.is_system"
				...
				<button
					...
					@click="$router.push('/operation/result-code/create')"
					class="btn btn-sm btn-rounded btn-primary i-con-h-a ml-2"
				>
					{{ $t("menu.modules.result_code_create.title") }}
						...
						<div slot="header">
							<h5 class="mb-0">
								...
								<span
									...
									class="badge badge-pill ml-1 badge-primary"
									>Aktif</span
								>
								<span v-else class="badge badge-pill ml-1 badge-light"
									>Pasif</span
								>
							</h5>
							<small
